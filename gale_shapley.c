 ...
 * 남자와 여자는 모두 0 부터 n-1 까지의 정수로 표현된다. (프로그램 내부에서는)
...
{    
    int n; // 짝의 수
    printf("Enter number of pairs you want to make: "); scanf("%d", &n);
...
void gale_shapley(int n, int man_pref[][n], int woman_pref[][n], int pair_man[n], int pair_woman[n], bool is_man_married[n])
{
    ...
    while (1) {
        ...
        if (curr_fiance == free) { // man_pref[i][j] 가 현재 약혼하지 않은 경우 (즉 신랑이 존재하지 않는 경우)
...
/***********************************************************************************
 index_finder: 길이 n인 1차원 리스트에서 i라는 원소가 존재하면 그 i라는 원소가 존재하는
               곳의 index를 반환한다. 만약 그 원소가 존재하지 않으면 -1을 반환한다.
************************************************************************************/
int index_finder(int n, int list[n], int i)
{
    int *j = list;
